...
import treas from 'public/coins.gif'
import starss from 'public/star-rating-morph.gif'
import firr from 'public/output-onlinegiftools.gif'
import bronzeBadge from 'public/bronze.png';
import silverBadge from 'public/silver.png';
import goldBadge from 'public/gold.png';
import platinumBadge from 'public/platinum.png';
...
const getLingotName = (lingots: number) => {
  if (lingots === 1) return "Bronze";
  if (lingots === 2) return "Silver";
  if (lingots === 3) return "Gold";
  if (lingots === 4) return "Platinum";
  return "";
};

const getBadgeImageForLingot = (lingots: number) => {
  if (lingots === 1) return bronzeBadge;
  if (lingots === 2) return silverBadge;
  if (lingots === 3) return goldBadge;
  if (lingots === 4) return platinumBadge;
  return null;
};
        ...
        <article className="my-6 flex">
          ...
          >
            <div className="pointer-events-none">
              {streak > 0 ? <Image src={firr} width="45" height="40" alt={""} /> : <EmptyFireSvg />}
            </div>
            ...
            >
              ...
              <p className="text-center text-sm font-normal text-gray-400">
                {`Beware! If you miss tomorrow's practice, your streak will reset. Be vigilant!`}
              </p>
          ...
          >
            ...
            >
              ...
              <div className="flex flex-col gap-3">
                ...
                You have {lingots} {lingots === 1 ? "gem" : "gems"}.
                </p>
              </div>
            </div>
          </span>
...
const UnlockLeaderboardsSection = () => {
  ...
  return (
    <article className="flex flex-col gap-5 rounded-2xl border-2 border-gray-700 p-6 text-gray-700">
      <h2 className="text-xl font-bold">Unlock Leaderboards!</h2>
      <div className="flex items-center gap-6">
        <LockedLeaderboardsSvg />
        <p className="text-sm leading-6 text-gray-500">
          {lessonsNeededToUnlockLeaderboards === 1 ? "" : ""}
        </p>
      </div>
    </article>
  );
    ...
    <article className="flex flex-col gap-5 rounded-2xl border-2 border-gray-200 p-6 text-gray-700">
      ...
      <div className="flex gap-6">
        ...
        <div className="flex flex-col gap-5">
          ...
          <p className="text-sm leading-6 text-gray-500">
            You earned {xpThisWeek} LP this week so far
          </p>
        </div>
      </div>
    </article>
...
const DailyQuestsSection = () => {
  ...
  const badgeName = getLingotName(lingots);
  const badgeImage = getBadgeImageForLingot(lingots);
  return (
    <article className="flex flex-col gap-5 rounded-2xl border-2 border-gray-700 p-6 font-bold text-white">
      <h2 className="text-xl text-gray-700">Badges</h2>
      <div className="flex flex-col gap-3">
        {lingots > 0 && (
                  <div className="flex flex-col gap-3 justify-start items-start">
                    <span className="text-sm font-normal text-gray-700">
                      Congratulations you have gained a
                    </span>
                    <span className={`text--${getLingotName(lingots).toLowerCase()}-500 text-sm text-gray-700 font-normal`}>
                      {getLingotName(lingots)} badge!
                    </span>
                    <div style={{marginBottom:'10px',marginLeft:'250px',marginTop:'-70px'}}>
                    <img src={badgeImage?.src ?? ''} alt={badgeName} className="w-16 h-16" />
                    </div>
                  </div>
                )}
        </div>
...
const XpProgressSection = () => {
  const xpToday = useBoundStore((x) => x.xpToday());
  const goalXp = useBoundStore((x) => x.goalXp);
  // const [correctAnswersAccuracy, setCorrectAnswersAccuracy] = useState<number>(0);
  // const calculateCorrectAnswersAccuracy = () => {
  //   // Replace this with your actual logic to calculate the correct answers accuracy
  //   // For example, you can get the data from your store or API and calculate the percentage.
  //   const accuracy = 75; // Replace this with the actual value
  //   setCorrectAnswersAccuracy(accuracy);
  //   calculateCorrectAnswersAccuracy();
  // };
  return (
    <article className="flex flex-col gap-5 rounded-2xl border-2 border-gray-700 p-6 font-bold text-gray-700">
      <div className="flex items-center justify-between">
        <h2 className="text-xl">LP Progress</h2>
        <Link href="/settings/coach" className="uppercase text-gray-700">
          Edit goal
        </Link>
      </div>
      <div className="flex gap-5">
        <Image src={treas} width="60" height="20" alt={""}/>
        <div className="flex grow flex-col justify-around">
          <h3 className="font-normal text-gray-700">Daily goal</h3>
          <div className="flex items-center gap-5">
            ...
            <div className="text-md shrink-0 font-normal text-gray-700">
              {xpToday}/{goalXp} LP
            </div>
          </div>
        </div>
      ...
      <div className="flex gap-5">
        <div className="flex grow flex-col justify-around">
          {/* <h3 className="font-normal text-gray-700">Correct Answers Accuracy</h3> */}
          <div className="flex items-center gap-5">
            {/* <div className="relative h-4 w-full grow rounded-full bg-gray-200">
              {correctAnswersAccuracy > 0 && (
                <div
                  className="absolute left-0 top-0 h-4 rounded-full bg-green-400"
                  style={{
                    width: `${Math.min(1, correctAnswersAccuracy / 100) * 100}%`,
                  }}
                >
                  <div className="absolute left-2 top-1 right-2 h-[6px] rounded-full bg-green-300"></div>
                </div>
              )}
            </div>
            <div className="text-md shrink-0 font-normal text-gray-700">
              {correctAnswersAccuracy.toFixed(1)}% Accuracy
            </div> */}
          </div>
        </div>
      </div>
...
// }) => {
//   return (
//     <article className="flex flex-col gap-5 rounded-2xl border-2 border-gray-700 p-6 font-bold">
//       <h2 className="text-xl text-gray-700">Create a profile to save your progress!</h2>
//       <button
//         className="rounded-2xl border-b-4 bg-yellow-300 py-3 uppercase text-white transition hover:border-yellow-500 hover:bg-yellow-400"
//         onClick={() => setLoginScreenState("SIGNUP")}
//       >
//         Create a profile
//       </button>
//       <button
//         className="rounded-2xl border-b-4 bg-blue-400 py-3 uppercase text-white transition hover:border-blue-400 hover:bg-blue-300"
